...
Versão 2 — otimizada para eficiência computacional com RAM limitada.

Arquitetura: Fully Convolutional Network — Wang et al. (2017)

Para rodar:
    python scripts/train_cnn1d_v2.py
"""
...
def main() -> None:
    ...
    # Usa .pop() para liberar a parte pandas imediatamente após processar cada instância,
    # mantendo uso de RAM constante (~tamanho de instance_arrays) durante toda a fase.
    ...
    for fold, (train_idx, test_idx) in enumerate(
        ...
        np.save(METRICS_DIR / "cnn1d_oof_true.npy", np.concatenate(oof_true))
        np.save(METRICS_DIR / "cnn1d_oof_pred.npy", np.concatenate(oof_pred))
