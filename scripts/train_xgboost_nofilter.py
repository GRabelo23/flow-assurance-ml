"""
XGBoost — Estado Operacional sem filtragem de sinal (dados brutos).

Gera as features a partir do cleaned.parquet com smooth_filter='none'
(sem Gaussiano nem filtro estatístico) e treina o XGBoost com o mesmo
procedimento do script principal, permitindo comparação direta.

Execução:
    python scripts/train_xgboost_nofilter.py

Saídas:
    data/processed/features_nofilter_window_class.parquet
    results/models/xgboost_nofilter_window_class.joblib
    results/models/imputer_nofilter_window_class.joblib
    results/metrics/xgboost_nofilter_metrics.json
"""
...
MODEL_PATH             = MODELS_DIR / "xgboost_nofilter_window_class.joblib"
IMPUTER_PATH           = MODELS_DIR / "imputer_nofilter_window_class.joblib"
METRICS_PATH           = METRICS_DIR / "xgboost_nofilter_metrics.json"


def gerar_features():
    print("\n[1/5] Gerando features SEM filtro (smooth_filter='none')...")
    print(f"  Lendo: {CLEANED_DATA_PATH}")
    run_pipeline_from_cleaned(
        cleaned_path=CLEANED_DATA_PATH,
        features_path=FEATURES_NOFILTER_PATH,
        label_strategy="window",
        smooth_filter="none",
        verbose=True,
    )
    print(f"  Features salvas: {FEATURES_NOFILTER_PATH}")


def main():
    print("=" * 65)
    print("  XGBoost — Estado Operacional  |  sem filtragem de sinal")
    print(f"  Inicio: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 65)

    # ── Etapa 1: gerar features se necessário ────────────────────────────────
    if FEATURES_NOFILTER_PATH.exists():
        print(f"\n[1/5] Features ja existem: {FEATURES_NOFILTER_PATH}")
    ...
    # ── Comparação direta com o modelo filtrado ───────────────────────────────
    ref_path = METRICS_DIR / "xgboost_window_class_metrics.json"
    if ref_path.exists():
        ...
        print(f"\n  Comparacao com XGBoost (filtro Gaussiano):")
        print(f"    Gaussiano : {ref_f1:.4f}")
        print(f"    Sem filtro: {f1_macro:.4f}  (delta = {sinal}{delta})")

    print_classification_report(y_true_all, y_pred_all,
                                 model_name="XGBoost Sem Filtro")
    ...
    metrics = {
        "model":      "xgboost_nofilter_window_class",
        "filter":     "none",
